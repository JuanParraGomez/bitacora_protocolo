...
`pages/tasks/[id].vue` is the page-root composition for the shell. It owns route hydration, storage batch persistence, project selection, and overlay-query sync while delegating business UI to `TaskWorkspace`. The route keeps the workspace mounted for preserved deep links:

- `/tasks/:id` renders the baseline guided workspace.
- `/tasks/:id?overlay=new-task&projectId=<id>` opens task creation on top of the active workspace.
- `/tasks/:id?overlay=library` and `/tasks/:id?overlay=library&record=<id>` open the library slideover without leaving the current task context.
- `/tasks/:id?overlay=settings` opens the assistant settings modal from the same shell.

The shared compatibility batch contract is intentionally narrow: exact keys `bitacora:index`, `bitacora:projects`, and `bitacora:assistant-settings`, plus prefix keys `bitacora:t:<id>` and `bitacora:r:<id>`. Anything else is rejected by the storage contract guard.
